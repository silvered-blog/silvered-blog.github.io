...
def seq_to_vec(seq, embedding_dict, k):     #改动：使用累加求和的方式，避免为超大基因组一次性分配过多内存
    ...
    for i in range(len(seq) - k + 1):
        ...
        if kmer in embedding_dict:
            if total is None:
                # 第一次命中时，初始化累加器为与词向量相同形状的零数组
                total = np.zeros_like(embedding_dict[kmer])
            total += embedding_dict[kmer]
            count += 1
...
if __name__ == "__main__":
    ...
    My_model = [r"", , r"", '', '']             # 前面的模型都不用管，把这个的参数和路径填写好就行了

    model = My_model
    # 使用示例
    parser = argparse.ArgumentParser(description="DNA序列益生菌预测工具")
    parser.add_argument("--input_dir", default='',  # 填.fasta文件路径
                        help="输入DNA序列文件夹")
